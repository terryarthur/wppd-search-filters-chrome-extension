...
let filteredPlugins = [];
let currentPage = 1;
let itemsPerPage = 24; // Balanced page size for faster load times
...
async function fetchPluginData(searchTerm = '') {
  const TIMEOUT_MS = 15000; // 15 second timeout for multiple pages
  const MAX_RETRIES = 3;
  const PER_PAGE = 100; // Fetch more per page for efficiency
  const MAX_PAGES = 50; // Limit to prevent excessive API calls (5000 plugins max)
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const url = `https://api.wordpress.org/plugins/info/1.2/?action=query_plugins&request[search]=${encodeURIComponent(searchTerm)}&request[per_page]=${PER_PAGE}&request[page]=1&request[fields][short_description]=true&request[fields][rating]=true&request[fields][ratings]=true&request[fields][active_installs]=true&request[fields][last_updated]=true&request[fields][icons]=true&request[fields][num_ratings]=true`;
      ...
      if (data && data.plugins && Array.isArray(data.plugins)) {
        ...
        if (data.info && data.info.results) {
          totalPages = Math.min(Math.ceil(data.info.results / PER_PAGE), MAX_PAGES);
          console.log(`Found ${data.info.results} total plugins, fetching ${totalPages} pages`);
        }
  ...
  if (totalPages > 1) {
    ...
    for (let page = 2; page <= totalPages; page++) {
      const pagePromise = fetchSinglePage(searchTerm, page, PER_PAGE, TIMEOUT_MS);
      pagePromises.push(pagePromise);
      
      // Batch requests to avoid overwhelming the API
      if (pagePromises.length >= 10 || page === totalPages) {
        try {
          const pageResults = await Promise.all(pagePromises);
          pageResults.forEach(pagePlugins => {
            if (Array.isArray(pagePlugins)) {
              allPlugins = allPlugins.concat(pagePlugins);
            }
          });
          ...
          console.warn('Some pages failed to load:', batchError.message);
  ...
  allPlugins.forEach(plugin => {
    if (plugin && plugin.slug && !seenSlugs.has(plugin.slug)) {
      seenSlugs.add(plugin.slug);
      uniquePlugins.push(plugin);
    }
  });
  
  console.log(`Successfully fetched ${allPlugins.length} plugins total, ${uniquePlugins.length} unique`);
...
// Helper function to fetch a single page of plugins
async function fetchSinglePage(searchTerm, page, perPage, timeout) {
  const url = `https://api.wordpress.org/plugins/info/1.2/?action=query_plugins&request[search]=${encodeURIComponent(searchTerm)}&request[per_page]=${perPage}&request[page]=${page}&request[fields][short_description]=true&request[fields][rating]=true&request[fields][ratings]=true&request[fields][active_installs]=true&request[fields][last_updated]=true&request[fields][icons]=true&request[fields][num_ratings]=true`;
  ...
  try {
    ...
    if (data && data.plugins && Array.isArray(data.plugins)) {
      ...
      console.warn(`Page ${page} returned unexpected format`);
      return [];
    }
  } catch (error) {
...
function clearFilters() {
  ...
  // Reset status - reload the correct plugin count
  setTimeout(async () => {
    await loadInitialPluginCount();
  }, 100);
}
...
async function init() {
  ...
  try {
    ...
    // Load API data and show correct count
    await loadInitialPluginCount();
...
// Load plugin count from API to match search results
async function loadInitialPluginCount() {
  const statusElement = document.getElementById('filter-status');
  statusElement.textContent = 'Loading plugin count...';
  statusElement.style.color = '#666';
  
  try {
    const searchTerm = getCurrentSearchTerm();
    console.log('Loading initial count for search term:', searchTerm);
    
    // Fetch first page to get total count quickly
    const url = `https://api.wordpress.org/plugins/info/1.2/?action=query_plugins&request[search]=${encodeURIComponent(searchTerm)}&request[per_page]=1&request[page]=1`;
    ...
    if (data && data.info && data.info.results) {
      const totalResults = data.info.results;
      console.log(`Found ${totalResults} total plugins for search "${searchTerm}"`);
      statusElement.textContent = `Ready to filter ${totalResults.toLocaleString()} plugins`;
      statusElement.style.color = '#666';
    } else {
      ...
      setTimeout(() => {
        ...
        statusElement.textContent = `Ready to filter ${visibleCards.length} plugins`;
      }, 100);
    }
  } catch (error) {
